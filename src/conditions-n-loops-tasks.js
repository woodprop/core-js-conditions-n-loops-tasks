...
function convertToRomanNumerals(num) {
  ...
  function getLast(digit) {
    let res = '';
    if (base[digit]) return base[digit];
    if (digit < 5) {
  ...
  if (num <= 10) return getLast(num);

  let res = '';
  for (let i = 0; i < Math.floor(num / 10); i += 1) {
    res += base[10];
  }
  return res + getLast(num % 10);
}
...
function getBalanceIndex(arr) {
  ...
  for (let i = 1; i < arr.length; i += 1) {
    if (getSum(i, 0) === getSum(i, 1)) return i;
  }
  return -1;
}
...
function getSpiralMatrix(size) {
  ...
  for (let l = 0; l <= Math.ceil(size / 2); l += 1) {
    makeRound(size, l);
  }
...
function shuffleChar(str, iterations) {
  ...
  while (iter > 0) {
    let even = '';
    let odd = '';
    for (let n = 1; n < str.length; n += 2) {
      even += newStr[n - 1];
      odd += newStr[n];
    }
    newStr = even + odd;
    iter -= 1;
    if (newStr === str) iter = iterations % (iterations - iter);
  }
  return newStr;
}
